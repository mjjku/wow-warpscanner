...
install_termux_tools() {
    ...
    if ! command -v termux-toast >/dev/null 2>&1; then
        if ! pkg_install "termux-tools"; then
...
check_termux_environment() {
    ...
    if [ $attempt -gt $MAX_RETRIES ]; then
        ...
        echo -e "${YELLOW}⚠️ Please grant storage permissions manually via Termux settings (Settings > Apps > Termux > Permissions > Storage) or run 'termux-setup-storage' manually and restart the script.${NC}"
        log_error "Failed to obtain storage permissions"
        exit 1
    fi
    
    # Create storage directory with retry
    attempt=1
    while [ $attempt -le $MAX_RETRIES ]; do
        if mkdir -p "$TERMUX_STORAGE_DIR" 2>/dev/null; then
            echo -e "${GREEN}✅ Storage directory created at $TERMUX_STORAGE_DIR.${NC}"
            break
        else
            echo -e "${YELLOW}⚠️ Failed to create storage directory (attempt $attempt/$MAX_RETRIES). Retrying...${NC}"
            if command -v termux-toast >/dev/null 2>&1; then
                termux-toast -g top "Failed to create storage directory. Please grant storage permissions." &
            else
                echo -e "${YELLOW}⚠️ Please ensure storage permissions are granted and retry.${NC}"
            fi
            sleep $RETRY_DELAY
            ((attempt++))
        fi
    ...
    if [ $attempt -gt $MAX_RETRIES ]; then
        echo -e "${RED}❌ Failed to create storage directory $TERMUX_STORAGE_DIR after $MAX_RETRIES attempts.${NC}"
        echo -e "${YELLOW}⚠️ Please run 'termux-setup-storage' manually, grant permissions, and restart the script.${NC}"
...
patch_imports() {
    local target_path="$CONFIG_DIR/$SCRIPT_NAME"
    if [ -f "$target_path" ] && ! grep -q "X25519PrivateKey" "$target_path" 2>/dev/null; then
        echo -e "${YELLOW}⚙️ Adding missing imports to WarpFusion script...${NC}"
        sed -i '1i from cryptography.hazmat.primitives.asymmetric.x25519 import X25519PrivateKey' "$target_path" 2>/dev/null
        sed -i '2i from cryptography.hazmat.primitives import serialization' "$target_path" 2>/dev/null
    fi
}
...
run_warpfusion() {
    ...
    if [[ "$env" == "ubuntu" ]]; then
        ...
        local attempt=1
        while [ $attempt -le $MAX_RETRIES ]; do
            if python3 "$target_path" 2>/dev/null; then
...
if ! bash -n "$0" 2>/dev/null; then
    echo -e "${RED}❌ Script contains syntax errors. Please check the file and try again.${NC}"
    exit 1
fi
