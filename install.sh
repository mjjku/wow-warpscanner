...
check_termux_environment() {
    ...
    # Ensure termux-tools is installed first
    if ! command -v termux-toast >/dev/null 2>&1; then
        echo -e "${BLUE}📦 Installing termux-tools...${NC}"
        pkg_install termux-tools || {
            echo -e "${RED}❌ Failed to install termux-tools. Please install it manually.${NC}"
    ...
    while [ $attempt -le $MAX_RETRIES ]; do
        ...
        if [ $attempt -gt $MAX_RETRIES ]; then
            ...
            echo -e "${YELLOW}⚠️ Please grant storage permissions manually via Termux settings.${NC}"
...
install_termux_dependencies() {
    ...
    for pkg in "${APT_PACKAGES[@]}"; do
        if ! command -v "$pkg" >/dev/null 2>&1 && [[ "$pkg" != "pkg-config" ]] && [[ "$pkg" != "termux-tools" ]] && [[ "$pkg" != "proot-distro" ]]; then
            ...
            pkg_install "$pkg" &
            jobs+=($!)
        elif [[ "$pkg" == "proot-distro" ]] && ! command -v proot-distro >/dev/nu
ll 2>&1; then
            pkg_install "$pkg" &
            jobs+=($!)
        fi
...
setup_ubuntu() {
    ...
    # Update Ubuntu packages and install dependencies in parallel
...
check_network() {
    ...
    local attempt=1
    while [ $attempt -le $MAX_RETRIES ]; do
        if timeout $TIMEOUT wget -q --spider http://1.1.1.1; then
