...
# Version: 15.2.0 (Enhanced Persian Edition)
# Description: نصب کامل خودکار، بدون خطا با قابلیت بازیابی پیشرفته و بهینه‌سازی حرفه‌ای
...
__emergency_recovery() {
    ...
    pkg install -y proot-distro >/dev/null 2>&1 || {
        __show_error "نصب proot-distro در بازیابی ناموفق بود"
        exit 1
    }
}
...
__check_environment() {
    ...
    if ! termux-setup-storage >/dev/null 2>&1; then
        __show_warning "دسترسی به ذخیره‌سازی فعال شد"
    fi
    ...
    # اطمینان از وجود pkg-config
    if ! command -v pkg-config >/dev/null 2>&1; then
        pkg install -y pkg-config >/dev/null 2>&1 || {
...
__install_core_dependencies() {
    ...
    # تنظیم مخازن برای پایداری
    echo "deb [trusted=yes] http://deb.debian.org/debian bookworm main" > /data/data/com.termux/files/usr/etc/apt/sources.list
    
    # تلاش چندباره برای بروزرسانی
    local max_retries=3
    local attempt=1
    ...
    for pkg in "${packages[@]}"; do
        if ! pkg list-installed | grep -q "$pkg"; then
            if ! pkg install -y "$pkg" -o Acquire::ForceIPv4=true >/dev/null 2>&1; then
                __show_warning "خطا در نصب $pkg، تلاش مجدد..."
                pkg install -y "$pkg" -o Acquire::ForceIPv4=true >/dev/null 2>&1 || {
...
__install_python_dependencies() {
    ...
    for package in "${py_packages[@]}"; do
        if ! pip install "$package" --no-warn-script-location >/dev/null 2>&1; then
...
__main() {
    ...
    echo " ███▌ ╒══════════════════════╕  ▐████ "
    echo " ███▌ │ نسخه ۱۵.۲.۰ (بهینه‌سازی شده) │  ▐████ "
    echo " ███▌ ╘══════════════════════╛  ▐████ "
